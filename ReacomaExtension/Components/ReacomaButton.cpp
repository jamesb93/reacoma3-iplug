...
ReacomaButton::ReacomaButton(const IRECT &bounds, const char *label,
                             ...
                             const IColor &bgColor, const IColor &fgColor,
                             const IColor &pressedColor,
                             const IColor &hoverColor, const IText &textStyle,
                             float frameThickness, float cornerRadius)
    : IButtonControlBase(bounds, actionFunction), mLabel(label),
      mTextStyle(textStyle), mBackgroundColor(bgColor),
      mForegroundColor(fgColor), mPressedColor(pressedColor),
      mHoverColor(hoverColor), mCurrentBackgroundColor(bgColor),
      mFrameThickness(frameThickness), mCornerRadius(cornerRadius),
      mIsPressed(false), mAnimationDurationMs(80) {
    mTextStyle.mFGColor = mForegroundColor;
    SetTooltip(label);
}

void ReacomaButton::SetColors(const IColor &bgColor, const IColor &fgColor,
                              const IColor &pressedColor,
...
void ReacomaButton::Draw(IGraphics &g) {
    ...
    if (mCornerRadius > 0.f) {
        g.FillRoundRect(mCurrentBackgroundColor, areaToDraw, mCornerRadius,
                        &mBlend);
        if (mFrameThickness > 0.f)
            g.DrawRoundRect(mForegroundColor, areaToDraw, mCornerRadius,
                            &mBlend, mFrameThickness);
    } else {
...
void ReacomaButton::AnimateState(bool isOver, bool isPressed) {
    ...
    if (startColor.R != targetColor.R) {
        SetAnimation(
            [this, startColor, targetColor](IControl *pCaller) {
                float progress = static_cast<float>(GetAnimationProgress());
                mCurrentBackgroundColor = IColor::LinearInterpolateBetween(
