...
void NMFAlgorithm::RegisterParameters() {
    ...
    mApiProvider->GetParam(mBaseParamIdx + kComponents)->InitInt("Number of Components", 2, 2, 10);
    mApiProvider->GetParam(mBaseParamIdx + kIterations)->InitInt("Number of Iterations", 100, 1, 1000);
}

bool NMFAlgorithm::DoProcess(InputBufferT::type& sourceBuffer, int numChannels, int frameCount, int sampleRate) {
    auto componentsParam = mApiProvider->GetParam(mBaseParamIdx + kComponents)->Value();
    auto iterationsParam = mApiProvider->GetParam(mBaseParamIdx + kIterations)->Value();
